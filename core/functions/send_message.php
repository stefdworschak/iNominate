    ...
    try{
      ...
      if(!empty($_POST) && $_SESSION['xssid'] == $_POST['xssid']) {
          ...
          if(empty($_POST['subject']) || empty($_POST['message'])) {
              header('Location:../../index.php?view=profile&id=' . $_POST['profile_id'] . '&send=missing_fields');
          //Otherwise add the message
          } else {
            $c = new DBClass;
            $result=$c->sendMessage($_POST['from_id'], $_POST['to_id'], $_POST['subject'], $_POST['message'], $_POST['thread_id']);
            if($result == true){
                header('Location:../../index.php?view=profile&id=' . $_POST['profile_id'] . '&send=success');
            } else {
               header('Location:../../index.php?view=profile&id=' . $_POST['profile_id'] . '&send=error');
            }
          }
